...
def get_arrows(size=(12, 12), show=True):
    train_labels = np.load(f'{ROOT_PATH}/arrows/labels_arr_train.npz')
    train_labels = train_labels.f.arr_0
    train_labels = np.expand_dims(train_labels, axis=-1)

    test_labels = np.load(f'{ROOT_PATH}/arrows/labels_arr_test.npz')
    test_labels = test_labels.f.arr_0
    test_labels = np.expand_dims(test_labels, axis=-1)

    train_img = np.load(f'{ROOT_PATH}/arrows/aug_arr_train.npz')
    ...
    test_img = np.load(f'{ROOT_PATH}/arrows/aug_arr_test.npz')
